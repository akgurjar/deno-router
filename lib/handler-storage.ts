...
export interface Handle {
  ...
  handler: Handler;
  store: unknown;
  _createParamsObject: (paramsArray: unknown[]) => unknown;
}

export default class HandlerStorage {
  unconstrainedHandler: Handle | null = null; // optimized reference to the handler that will match most of the time
  constraints: string[] = [];
  handles: Handle[] = []; // unoptimized list of handler objects for which the fast matcher function will be compiled
  constrainedHandlerStores: Record<string, unknown> | null = null;

  #getHandlerMatchingConstraints = (
    _constraints: Record<string, unknown>,
  ): Handle | null => null;

  // This is the hot path for node handler finding -- change with care!
  getMatchingHandler(derivedConstraints?: Record<string, unknown>) {
    if (derivedConstraints === undefined) {
      return this.unconstrainedHandler;
    }
    return this.#getHandlerMatchingConstraints(derivedConstraints);
  }

  addHandler(constrainer: Constrainer, route: Route) {
  ...
  _constrainedIndexBitmask(constraint: string) {
    let mask = 0;
    for (let i = 0; i < this.handles.length; i++) {
      const handle = this.handles[i];
      const constraintValue = handle.constraints[constraint];
      if (constraintValue !== undefined) {
        mask |= 1 << i;
      }
    }
  ...
  _compileGetHandlerMatchingConstraints(constrainer: Constrainer) {
    ...
    const lines = [];
    lines.push(`
    let candidates = ${(1 << this.handles.length) - 1}
    let mask, matches
    `);
    for (const constraint of this.constraints) {
      ...
      const matchMask = strategy.mustMatchWhenDerived
        ? "matches"
        : "(matches | mask)";
    ...
    for (const constraint in constrainer.strategies) {
      ...
      const strategy = constrainer.strategies[constraint];
      if (
        strategy.mustMatchWhenDerived && !this.constraints.includes(constraint)
      ) {
        lines.push(
          `if (derivedConstraints.${constraint} !== undefined) return null`,
        );
      }
    }
    ...
    this.#getHandlerMatchingConstraints = new Function(
      "derivedConstraints",
      lines.join("\n"),
    ) as (derivedConstraints: Record<string, unknown>) => Handle | null;
  }
}
