 ...
 */
function safeDecodeURI(path: string, useSemicolonDelimiter: boolean) {
  ...
  for (let i = 1; i < path.length; i++) {
    ...
    if (charCode === 37) {
      ...
      charCode === 63 || charCode === 35 ||
      (charCode === 59 && useSemicolonDelimiter)
    ) {
...
function safeDecodeURIComponent(uriComponent: string) {
  ...
  return uriComponent.slice(0, startIndex) + decoded +
    uriComponent.slice(lastIndex);
}

export { safeDecodeURI, safeDecodeURIComponent };
