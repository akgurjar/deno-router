...
Deno.test("pretty print commonPrefix - handle method constraint", async () => {
  ...
  findMyWay.on(
    "GET",
    "/test/:hello",
    { constraints: { method: "bar" } },
    () => {},
  );
  findMyWay.on(
    "GET",
    "/test/:hello",
    { constraints: { method: "baz" } },
    () => {},
  );
...
Deno.test("pretty print includeMeta - buildPrettyMeta function", async () => {
  ...
  const findMyWay = FindMyWay({
    buildPrettyMeta: (route) => {
      return {
        metaKey: route.method === "PUT" ? "Hide PUT route path" : route.path,
      };
    },
  });
  const namedFunction = () => {};
