...
Deno.test('A route could support a custom constraint strategy while versioned', async () => {
  ...
  deepEqual(findMyWay.find('GET', '/', { requestedBy: 'curl', version: '1.x' }).handler, alpha)
  deepEqual(findMyWay.find('GET', '/', { requestedBy: 'curl', version: '2.x' }).handler, beta)
  deepEqual(findMyWay.find('GET', '/', { requestedBy: 'wget', version: '2.x' }).handler, gamma)
  deepEqual(findMyWay.find('GET', '/', { requestedBy: 'wget', version: '3.x' }).handler, delta)

  assert(!findMyWay.find('GET', '/', { requestedBy: 'chrome' }))
  assert(!findMyWay.find('GET', '/', { requestedBy: 'chrome', version: '1.x' }))
...
Deno.test('A route could support a custom constraint strategy while versioned and host constrained', async () => {
  ...
  deepEqual(findMyWay.find('GET', '/', { requestedBy: 'curl', version: '1.x', host: 'fastify.io' }).handler, alpha)
  deepEqual(findMyWay.find('GET', '/', { requestedBy: 'curl', version: '2.x', host: 'fastify.io' }).handler, beta)
  deepEqual(findMyWay.find('GET', '/', { requestedBy: 'curl', version: '2.x', host: 'example.io' }).handler, delta)

  assert(!findMyWay.find('GET', '/', { requestedBy: 'chrome' }))
  assert(!findMyWay.find('GET', '/', { requestedBy: 'chrome', version: '1.x' }))
  assert(!findMyWay.find('GET', '/', { requestedBy: 'curl', version: '1.x' }))
  assert(!findMyWay.find('GET', '/', { requestedBy: 'curl', version: '2.x' }))
  assert(!findMyWay.find('GET', '/', { requestedBy: 'curl', version: '3.x', host: 'fastify.io' }))
  assert(!findMyWay.find('GET', '/', { requestedBy: 'curl', version: '1.x', host: 'example.io' }))
})
...
Deno.test('Custom constraint strategies can set mustMatchWhenDerived flag to true which prevents matches to unconstrained routes when a constraint is derived when there are constrained routes', async () => {
  ...
  findMyWay.on('GET', '/', {}, () => t.asserfail())
  findMyWay.on('GET', '/', { constraints: { requestedBy: 'curl' } }, () => t.asserfail())
  findMyWay.on('GET', '/', { constraints: { requestedBy: 'wget' } }, () => t.asserfail())
...
Deno.test('Has constraint strategy method test', async () => {
  ...
  deepEqual(findMyWay.hasConstraintStrategy(customHeaderConstraint.name), false)
  findMyWay.addConstraintStrategy(customHeaderConstraint)
