...
const Constrainer = await import("../lib/constrainer.ts").then((m) =>
  m.default
);
const { safeDecodeURIComponent } = await import("../lib/url-sanitizer.ts");
const acceptVersionStrategy = await import(
  "../lib/strategies/accept-version.ts"
).then((m) => m.default);
...
Deno.test("Constrainer.noteUsage", async () => {
  ...
  constrainer.noteUsage({ host: "jsr.io" });
...
Deno.test("Major version must be a numeric value", async () => {
  ...
  assertThrows(
    () =>
      findMyWay.on("GET", "/test", { constraints: { version: "x" } }, () => {}),
