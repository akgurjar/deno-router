...
Deno.test('A route could support multiple versions (find) / 1', async () => {
  ...
  assert(findMyWay.find('GET', '/', { version: 'application/vnd.example.api+json;version=2' }))
  assert(findMyWay.find('GET', '/', { version: 'application/vnd.example.api+json;version=3' }))
  assert(!findMyWay.find('GET', '/', { version: 'application/vnd.example.api+json;version=4' }))
  assert(!findMyWay.find('GET', '/', { version: 'application/vnd.example.api+json;version=5' }))
  assert(!findMyWay.find('GET', '/', { version: 'application/vnd.example.api+json;version=6' }))
})
...
Deno.test('Overriding custom strategies throws as error (add strategy outside constructor)', async () => {
  ...
  assertThrows(() => findMyWay.addConstraintStrategy(customVersioning),
