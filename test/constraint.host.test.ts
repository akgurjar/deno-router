...
Deno.test('A route supports multiple host constraints', async () => {
  ...
  deepEqual(findMyWay.find('GET', '/', { host: 'something-else.io' }).handler, alpha)
...
Deno.test('A route supports wildcard host constraints', async () => {
  ...
  deepEqual(findMyWay.find('GET', '/', { host: 'fastify.io' }).handler, beta)
  deepEqual(findMyWay.find('GET', '/', { host: 'foo.fastify.io' }).handler, gamma)
  deepEqual(findMyWay.find('GET', '/', { host: 'bar.fastify.io' }).handler, gamma)
...
Deno.test('A route supports multiple host constraints (lookup)', async () => {
  ...
  findMyWay.on('GET', '/', { constraints: { host: 'fastify.io' } }, (req, res) => {
    deepEqual(req.headers.host, 'fastify.io')
  })
