...
Deno.test('A route could support multiple host constraints while versioned', async () => {
  ...
  deepEqual(findMyWay.find('GET', '/', { host: 'fastify.io', version: '1.x' }).handler, beta)
  deepEqual(findMyWay.find('GET', '/', { host: 'fastify.io', version: '1.1.x' }).handler, beta)
  deepEqual(findMyWay.find('GET', '/', { host: 'fastify.io', version: '2.x' }).handler, gamma)
  deepEqual(findMyWay.find('GET', '/', { host: 'fastify.io', version: '2.1.x' }).handler, gamma)
  assert(!findMyWay.find('GET', '/', { host: 'fastify.io', version: '3.x' }))
  assert(!findMyWay.find('GET', '/', { host: 'something-else.io', version: '1.x' }))
})
...
Deno.test('Constrained routes are matched before unconstrainted routes when the constrained route is added last', async () => {
  ...
  deepEqual(findMyWay.find('GET', '/', { host: 'fastify.io' }).handler, beta)
...
Deno.test('Routes with multiple constraints are matched before routes with one constraint when the doubly-constrained route is added last', async () => {
  ...
  deepEqual(findMyWay.find('GET', '/', { host: 'fastify.io' }).handler, alpha)
  deepEqual(findMyWay.find('GET', '/', { host: 'fastify.io', version: '1.0.0' }).handler, beta)
  deepEqual(findMyWay.find('GET', '/', { host: 'fastify.io', version: '2.0.0' }), null)
})
